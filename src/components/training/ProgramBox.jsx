...
function ProgramBox({ program }) {
  return (
    <div
      className="w-full max-w-[900px] flex flex-col sm:flex-row gap-8 items-start"
      id={`program${program.id}`}
    >
      <div className="aspect-[3.75/2] sm:aspect-[4/5] flex w-full max-w-[600px] min-w-[250px] bg-neutral-300 rounded-2xl overflow-hidden">
        {program.imgMain && (
      ...
      <div className="flex flex-col items-start">
        ...
        <p className="text-neutral-500 leading-tight">{program.description}</p>
        {program.bullets && (
          <ul className="w-full list-outside list-disc ml-8 flex flex-col py-4 gap-1">
        ...
        <div className="w-full flex flex-col pl-8 sm:pl-0 items-start sm:items-center sm:flex-row gap-4 my-8">
          {program.primaryBtn && (
            <Button secondary={true} title={program.primaryBtn.title} />
          )}
