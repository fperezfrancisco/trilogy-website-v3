import React from "react";
import Header from "../components/nav/Header";
import HeroSectionRight from "../components/main/HeroSectionRight";
import HomeReviewSection from "../components/main/HomeReviewSection";
import CtaSection from "../components/cta/CtaSection";
import Footer from "../components/footer/Footer";
import trainingImg from "../assets/trainingImg-min.png";
import PreseasonProgramSection from "../components/main/PreseasonProgramSection";

const PreseasonPage = () => {
  return (
    <div className="w-full h-full">
      <Header />
      <main className="w-full max-w-[2400px] mx-auto mt-[56px] my-0 flex flex-col">
        <HeroSectionRight
          mainTitle={"Trilogy Soccer"}
          specialTitleText={"Preseason Program"}
          subheading={"June 16th - July 10th, 2025"}
          heroImg={trainingImg}
        />
        <PreseasonProgramSection />

        <HomeReviewSection />
        <CtaSection />
      </main>
      <Footer />
    </div>
  );
};

export default PreseasonPage;
